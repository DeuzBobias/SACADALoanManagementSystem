...
function renderDashboard() {
  const activeLoans = state.activeLoans.filter((loan) => !['Completed', 'Defaulted'].includes(loan.status));
  const completedLoans = state.activeLoans.filter((loan) => loan.status === 'Completed');
  const delinquentLoans = state.activeLoans.filter((loan) => ['Delayed', 'Delinquent', 'Defaulted'].includes(loan.status));
  ...
  const totalFees = state.loanApplications.reduce((sum, appItem) => {
    if (!['Approved', 'Released', 'Active'].includes(appItem.status)) return sum;
    return sum + calculateLoan(appItem.computation).processingFee;
  }, 0);
  const totalInterest = state.loanApplications.reduce((sum, appItem) => {
    if (!['Approved', 'Released', 'Active'].includes(appItem.status)) return sum;
    return sum + calculateLoan(appItem.computation).totalInterest;
  }, 0);

  const statusRows = ['Draft', 'Pending', 'Approved', 'Released', 'Active', 'Completed', 'Delayed', 'Delinquent'].map((status) => ({
    label: status,
    value:
      state.loanApplications.filter((loan) => loan.status === status).length +
      state.activeLoans.filter((loan) => loan.status === status).length,
  }));

  const monthlyCollections = [
    { label: 'Jan', value: 0, display: formatCurrency(0) },
    { label: 'Feb', value: 0, display: formatCurrency(0) },
    { label: 'Mar', value: 0, display: formatCurrency(0) },
    { label: 'Apr', value: 0, display: formatCurrency(0) },
    { label: 'May', value: 0, display: formatCurrency(0) },
    { label: 'Jun', value: totalCollections, display: formatCurrency(totalCollections) },
  ];
  ...
  const monthlyReleases = [
    ...
    { label: 'Apr', value: 0, display: formatCurrency(0) },
    { label: 'May', value: 0, display: formatCurrency(0) },
    { label: 'Jun', value: 0, display: formatCurrency(0) },
  ];

  const genderRows = groupRows(state.members, 'gender');
  const civilRows = groupRows(state.members, 'civilStatus');
  const employmentRows = groupRows(state.members, 'businessType', 'Employment');

  return `
    <section class="page-header">
      <div>
        <h3>Office Dashboard</h3>
        <p>Quick view of members, loan accounts, collections, and status items for staff review.</p>
      </div>
      <div class="top-actions">
    ...
    <div class="grid stats-grid">
      ${statCard('Total Members', formatNumber(state.members.length), 'Active and pending KYC records')}
      ${statCard('Pending Loan Applications', formatNumber(state.loanApplications.filter((loan) => loan.status === 'Pending').length), 'Needs evaluation')}
      ${statCard('Approved Loans', formatNumber(state.loanApplications.filter((loan) => loan.status === 'Approved').length), 'Approved but not all released')}
      ${statCard('Active Loans', formatNumber(activeLoans.length), 'Currently monitored accounts')}
      ${statCard('Completed Loans', formatNumber(completedLoans.length), 'Fully paid accounts')}
      ${statCard('Delinquent Loans', formatNumber(delinquentLoans.length), 'Delayed, delinquent, or defaulted')}
      ${statCard('Total Loan Receivables', formatCurrency(totalReceivables), 'Remaining balances')}
      ${statCard('Total Collections', formatCurrency(totalCollections), 'Payments encoded')}
      ${statCard('Interest / Service Fee Collected', formatCurrency(totalInterest), 'Expected collection charge')}
      ${statCard('Processing Fees Collected', formatCurrency(totalFees), '1% of released principal')}
    </div>

    <div class="grid two-column" style="margin-top: 16px;">
      ${panel('Loan Status Breakdown', barChart(statusRows), 'Counts by status')}
      ${panel('Monthly Collections', miniColumnChart(monthlyCollections), 'Simple month-to-month view')}
    </div>

    <div class="grid three-column" style="margin-top: 16px;">
      ${panel('Monthly Loan Releases', miniColumnChart(monthlyReleases), 'Released principal by month')}
      ${panel('Members by Gender', barChart(genderRows), 'KYC profile summary')}
      ${panel('Civil Status / Employment Type', `${barChart(civilRows)}<hr />${barChart(employmentRows)}`, 'Readable demographics')}
    </div>
  `;
}
...
function renderMemberList() {
  ...
  const rows = sortRows(
    state.members.filter((member) => {
      const text = `${member.id} ${member.fullName} ${member.mobile} ${member.barangay}`.toLowerCase();
      return (
    ...
    <section class="panel">
      ...
      ${dataTable({
        columns: [
          ...
          { key: 'barangay', label: 'Barangay / Address', sortable: true, render: (row) => `${escapeHtml(row.barangay)}<br><span class="muted">${escapeHtml(row.presentAddress)}</span>` },
          { key: 'membershipStatus', label: 'Membership Status', sortable: true, render: (row) => statusBadge(row.membershipStatus) },
          { key: 'shareCapital', label: 'Share Capital', sortable: true, render: (row) => formatCurrency(row.shareCapital) },
          { key: 'timeDeposit', label: 'Time Deposit', sortable: true, render: (row) => formatCurrency(row.timeDeposit) },
          { key: 'activeLoanCount', label: 'Active Loan Count', sortable: true },
          {
            key: 'actions',
            label: 'Actions',
            render: (row) => `
  ...
  return `
    ...
    ${formSection(
      ...
      `<div class="form-grid">
        ...
        ${field('Default Penalty Amount', 'defaultPenalty', '50', { type: 'number' })}
...
function buildReportRows(type) {
  ...
  if (type === 'Member Registry Report') {
  ...
  if (type === 'Loan Application Report') {
  ...
  if (type === 'Delinquent Loans Report') {
    return dataTable({
      columns: [
        { key: 'accountNumber', label: 'Loan Account' },
        { key: 'borrowerName', label: 'Borrower' },
...
function updateMemberFromForm(form) {
  ...
  state.notice = `${target.fullName || target.id} was saved in the member list.`;
...
function handleClick(event) {
  ...
  if (action === 'save-member') {
    updateMemberFromForm(document.querySelector('#member-form'));
  }
