...
export function renderEnhancedDashboard(state) {
  ...
  const sexRows = countRows(members, 'gender');
  const civilRows = countRows(members, 'civilStatus');
  const barangayRows = countRows(members, 'barangay');
  ...
  const summaryCards = [
    ...
    ['Total Paid-Up Share Capital', formatCurrency(paidUpCapital), 'Current member capital'],
    ['Total Capital Subscription', formatCurrency(amountSubscribed), 'Subscribed shares'],
  ...
  return `
    ...
    ${dashboardSection(
      ...
      `<div class="grid two-column">
        ...
        ${chartCard('Members by Sex', donutChart(sexRows), 'Sex / gender categories', '#/members')}
        ${chartCard('Civil Status', donutChart(civilRows), 'Civil status share', '#/reports')}
        ${chartCard('Members by Barangay', horizontalBarChart(barangayRows), 'Horizontal only because barangay names need room', '#/reports')}
