...
const citizenshipChoices = ['Filipino', 'Dual Citizen', 'Foreign National'];
...
const coMakerRelationshipChoices = [
  '',
  'Spouse',
  'Parent',
  'Child',
...
const boardDecisionChoices = ['', 'Approved', 'Disapproved', 'Deferred', 'For Re-evaluation'];
const genderChoices = ['Male', 'Female'];
const membershipStatusChoices = ['Active', 'Inactive', 'Terminated'];
const membershipStatusFilterChoices = ['All', ...membershipStatusChoices];
  ...
  return `
    ...
    <section class="panel">
      <div class="toolbar" data-filter-group="memberFilters">
        ...
        ${field('Status', 'status', filters.status, { type: 'select', choices: membershipStatusFilterChoices })}
        ${field('Barangay', 'barangay', filters.barangay, { type: 'select', choices: ['All', ...barangays] })}
        ${field('Gender', 'gender', filters.gender, { type: 'select', choices: ['All', ...genderChoices] })}
        ...
        ${field('Specialization', 'specialization', filters.specialization || 'All', { type: 'select', choices: ['All', ...specializationChoices.filter(Boolean)] })}
      </div>
      ${columnVisibilityMenu([
      ...
      <div class="profile-heading">
        ...
        <div>
          ...
          <div class="profile-meta">
            ...
            <span>${escapeHtml(`Date Accepted: ${formatDate(member.dateAccepted)}`)}</span>
...
function renderMemberTab(member, tab) {
  ...
  if (tab === 'Personal Information') {
    return formSection(
      ...
      `<div class="form-grid">
        ...
        ${field('Gender', 'gender', member.gender, { type: 'select', choices: genderChoices, allowCustomValue: false })}
        ...
        ${field('Citizenship', 'citizenship', member.citizenship, { type: 'select', choices: citizenshipChoices })}
        ${field('Religion', 'religion', member.religion, { type: 'select', choices: religionChoices })}
        ${field('Highest Educational Attainment', 'education', member.education, { type: 'select', choices: educationChoices })}
  ...
  if (tab === 'Membership') {
    return formSection(
      ...
      `<div class="form-grid">
        ...
        ${field('Membership Status', 'membershipStatus', member.membershipStatus, { type: 'select', choices: membershipStatusChoices, allowCustomValue: false })}
  ...
  if (tab === 'Education and Specialization') {
    return formSection(
      'Education and Specialization',
      'Use standard choices so education and specialization reports stay consistent.',
      `<div class="form-grid">
        ${field('Highest Educational Attainment', 'education', member.education, { type: 'select', choices: educationChoices })}
        ${field('Specialization', 'specialization', member.specialization || '', { type: 'select', choices: specializationChoices })}
      </div>`,
    );
  }
  ...
  if (tab === 'Address') {
    return formSection(
      ...
      `<div class="form-grid">
        ...
        ${field('Barangay', 'barangay', member.barangay, { type: 'select', choices: memberBarangayChoices() })}
...
function renderLoanStep(application, member) {
  ...
  if (state.loanStep === 3) {
    return formSection(
      ...
      `<div class="form-grid">
        ...
        ${field('Co-maker Member ID', 'coMakerMemberId', application.coMakerMemberId, { type: 'select', choices: ['', ...state.members.map((item) => item.id)] })}
        ${field('Co-maker Contact Number', 'coMakerContact', application.coMakerContact)}
        ${field('Relationship to Borrower', 'coMakerRelationship', application.coMakerRelationship, { type: 'select', choices: coMakerRelationshipChoices })}
  ...
  return formSection(
    ...
    `<div class="form-grid">
      ...
      ${field('Credit Committee Recommendation', 'recommendation', application.recommendation, { type: 'select', choices: recommendationChoices })}
      ${field('Board Decision', 'boardDecision', application.boardDecision, { type: 'select', choices: boardDecisionChoices })}
  ...
  return `
    ...
    <section class="panel">
      <div class="toolbar reports-toolbar" data-filter-group="reportFilters">
        ...
        ${field('Status', 'status', filters.status, { type: 'select', choices: ['All', 'Active', 'Inactive', 'Terminated', 'Pending', 'Approved', 'Released', 'Completed', 'Delayed', 'Delinquent', 'Defaulted'] })}
      </div>
      ${rows}
    </section>
  `;
  ...
  return `
    ...
    ${formSection(
      ...
      `<div class="form-grid">
        ...
        ${field('Available Membership Statuses', 'availableMembershipStatuses', 'Active, Inactive, Terminated')}
      </div>`,
    )}
    ...
    ${formSection(
      ...
      `<div class="form-grid">
        ${field('Reporting Year', 'allocationYear', '2026', { type: 'select', choices: ['2022', '2023', '2024', '2025', '2026'] })}
...
function buildReportRows(type) {
  ...
  if (['Demographics Report', 'Membership by Type', 'Members by Barangay', 'Members by Sex', 'Members by Civil Status', 'Members by Age Group', 'Members by Profession', 'Members by Education', 'Members by Income Classification', 'Members by Specialization'].includes(type)) {
    const rows = [
      ...genderReportRows(state.members).map((row) => ({ type: 'Gender', ...row })),
...
function genderReportRows(rows) {
...
function mergedChoices(baseChoices, extraChoices = []) {
  return [...new Set([...baseChoices, ...extraChoices].filter((choice) => choice !== undefined && choice !== null))];
}

function memberBarangayChoices() {
  return mergedChoices(viganBarangayChoices, uniqueValues(state.members, 'barangay'));
}
...
function navigateToRoute(hash, { preserveScroll = false } = {}) {
  ...
  if (scrollPosition) {
    requestAnimationFrame(() => {
      window.scrollTo({ left: scrollPosition.left, top: scrollPosition.top });
      const sidebar = document.querySelector('#sidebar');
      if (sidebar) sidebar.scrollTop = scrollPosition.sidebarTop;
    });
  }
}
...
async function handleClick(event) {
  ...
  const moduleLink = event.target.closest('.nav-link[href^="#/"]');
  if (sortHeader) {
  ...
  if (moduleLink) {
    ...
    navigateToRoute(moduleLink.getAttribute('href'), { preserveScroll: true });
    return;
  }
