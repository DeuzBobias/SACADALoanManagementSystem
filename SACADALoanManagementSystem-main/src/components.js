import { formatCurrency, formatDate, formatNumber } from './calculations.js';
...
export function yearSelector(value = '2026') {
  return field('Reporting Year', 'year', value, {
    type: 'select',
    choices: ['All Years', '2022', '2023', '2024', '2025', '2026'],
  });
}
...
export function field(label, name, value = '', options = {}) {
  ...
  if (type === 'select') {
    ...
    if (allowCustomValue && String(value ?? '') && !selectChoices.some((choice) => String(choice) === String(value))) {
      selectChoices.unshift(value);
    }
    control = `
...
function compactNumber(value) {
  ...
  return new Intl.NumberFormat('en-PH', {
    ...
    maximumFractionDigits: Math.abs(number) >= 100000 ? 1 : 2,
  }).format(number);
}
...
function chartNumber(row, key = 'value') {
  const value = row[key];
  const display = row.display ?? '';
  if (String(display).includes('₱') || String(display).includes('PHP')) return compactCurrency(value);
  return compactNumber(value);
}

function chartExactNumber(row, key = 'value') {
  return row.display ?? formatNumber(row[key] || 0);
}
          ...
          (row) => `
            <div class="bar-row">
              <div class="bar-label">
                ...
                <span class="chart-value" title="${escapeHtml(chartExactNumber(row))}">${escapeHtml(chartNumber(row))}</span>
              </div>
...
export function lineChart(rows, series) {
  ...
  return `
    <div class="line-chart-wrap">
      <svg class="line-chart" viewBox="0 0 ${width} ${height}" role="img" aria-label="Trend chart">
        ...
        ${series
          .map((item, seriesIndex) => {
            ...
            const color = item.color || `var(--chart-${(seriesIndex % 6) + 1})`;
            return `
              <polyline points="${points}" style="stroke: ${escapeHtml(color)};" />
              ${rows
                .map(
                  (row, index) =>
                    `<circle cx="${x(index)}" cy="${y(row[item.key])}" r="4" style="fill: ${escapeHtml(color)};"><title>${escapeHtml(row.label)} ${escapeHtml(item.label)}: ${escapeHtml(row[item.key] ?? 0)}</title></circle>`,
                )
                .join('')}
            `;
          })
      ...
      <div class="chart-legend">
        ${series
          .map((item, index) => `<span><i style="background: ${escapeHtml(item.color || `var(--chart-${(index % 6) + 1})`)};"></i>${escapeHtml(item.label)}</span>`)
          .join('')}
      </div>
    </div>
  `;
...
export function donutChart(rows) {
  ...
  return `
    <div class="donut-wrap">
      <div class="donut" style="background: conic-gradient(${stops});" title="${escapeHtml(exactTotal)}">
        <span class="donut-value" aria-label="${escapeHtml(exactTotal)}">${escapeHtml(totalLabel)}</span>
      </div>
      <div class="chart-legend">
        ${rows
          .map((row, index) => {
            const pct = total ? (Number(row.value || 0) / total) * 100 : 0;
            const exactValue = chartExactNumber(row);
            const tooltip = row.tooltip || `${row.label}: ${exactValue}`;
            return `<span title="${escapeHtml(tooltip)}"><i style="background: var(--chart-${(index % 6) + 1});"></i><b class="chart-label">${escapeHtml(row.label)}:</b> <strong class="chart-value">${escapeHtml(chartNumber(row))} (${pct.toFixed(1)}%)</strong></span>`;
          })
          .join('')}
      </div>
    </div>
