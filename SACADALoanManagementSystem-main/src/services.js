...
const backendServices = {
  capitalTransactions: backendCapitalService,
  delinquentLoans: delinquencyService,
  timeDeposits: backendTimeDepositService,
  expenses: backendExpenseService,
  dividends: backendDividendService,
  patronage: backendPatronageService,
  birRegistry: backendBirRegistryService,
};

function findCachedMember(memberId) {
  return cache.members?.find((member) => member.id === memberId);
}
...
function normalizeGender(value) {
  return normalizeChoice(value, ['Male', 'Female']);
}

function normalizeMembershipStatus(value) {
  return normalizeChoice(value, ['Active', 'Inactive', 'Terminated'], 'Active');
}
...
function memberFromDb(row) {
  return {
    ...
    ...(row.extra_data || {}),
    gender: normalizeGender(row.extra_data?.gender || row.gender),
    membershipStatus: normalizeMembershipStatus(row.extra_data?.membershipStatus || row.membership_status),
    province: row.extra_data?.province || row.province || '',
  };
}
...
function memberToDb(member) {
  return {
    ...
    gender: emptyToNull(normalizeGender(member.gender)),
    ...
    membership_status: normalizeMembershipStatus(member.membershipStatus),
    extra_data: {
...
function capitalFromDb(row) {
  const member = findCachedMember(row.member_id);
  return {
    id: row.id,
    memberId: row.member_id || '',
    memberName: row.member_name || row.members?.full_name || member?.fullName || '',
...
function delinquentLoanFromDb(row) {
  ...
  const member = row.members || findCachedMember(row.member_id) || {};
  const borrower = loan.borrowerName || loan.borrower_name || loan.full_name || member.fullName || member.full_name || displayNameFromParts(loan) || displayNameFromParts(member);
  return {
    loanNumber: row.id,
    voucherNumber: loan.voucher_no || '',
    borrower: row.borrower || borrower,
    memberId: row.member_id || '',
    contact: row.contact_details || '',
    barangay: row.barangay || '',
    originalAmount: numberOrZero(loan.loan_amount),
    term: loan.loan_terms ? `${loan.loan_terms} months` : '',
    totalPayable: numberOrZero(loan.total_payable),
    monthlyPayment: numberOrZero(row.monthly_payment),
    totalPayments: numberOrZero(loan.loan_payments),
...
async function loadRemoteData(name) {
  ...
  if (backendServices[name]) {
    const rows = await backendServices[name].list();
    return (fromDbMappers[name] || ((items) => items))(rows || []);
  }
...
export async function initializeServices() {
  const memberResult = await Promise.allSettled([memberService.load()]);
  ...
  const results = [...memberResult, ...moduleResults];
  const names = Object.keys(serviceTables);
  return results
